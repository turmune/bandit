...
from .base import BaseEndToEndModule
...
class Bandit(BaseBandit):
    ...
    def separate(self, batch):
        ...
        # Upstream always decodes every stem. ``active_stems`` lets a caller ask
        # for a subset: the shared trunk above still runs (it is ~98% of the
        # compute), but the inference handler then buffers only the requested
        # stems, which is what actually constrains memory on a small host.
        active = getattr(self, "active_stems", None) or self.stems

        for stem in active:
            mem = self.mask_estim[stem]
            m = mem(q)
